...
type
  ...
  Argument* = ref object of RootObj
    ...
    of akSwitch:
      switch*: string
      abbreviation*: bool  # Is abbreviation?
...
proc getFlag*(args: seq[Argument], name: string): string =
  for arg in args:
    if arg.kind == akFlag:
      if arg.key == name:
        return arg.value
...
proc getUIntFlag*[T: SomeUnsignedInt](
  args: seq[Argument], 
  name: string, 
  default: T = default(T)
): T {.inline.} =
  let flag = args.getFlag(name)

  try:
    return parseUint(flag)
  except ValueError:
    return default
